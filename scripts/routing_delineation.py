...
class RoutingNetwork:
    ...
    def run(self):
        ...
        extraction_folder=os.path.join(os.getcwd(), gauge_name, 'extraction')
        ...
        filter_lake_folder = os.path.join(os.getcwd(), gauge_name, 'filter_lakes')
        ...
        interested_lake_ids = [] # Change this
        ...
        # define another folder that will save the outputs
        drainage_area_folder = os.path.join(os.getcwd(), gauge_name, 'drainage_area')
        ...
        basin_shp = gpd.read_file(project_root / gauge_name / 'drainage_area' / f'catchment_without_merging_lakes_{version_number}.shp')
        basin_shp = basin_shp.to_crs("EPSG:4326")
        basin_shp['Gauge_ID'] = gauge_name
        basin_shp = basin_shp.dissolve(by='Gauge_ID').reset_index()[['Gauge_ID','geometry']]
        basin_shp.to_file(project_root / gauge_name /'tmp_basin_outline.shp')

        # Get the bounding box of the basin outline
        shapefile = fiona.open(project_root / gauge_name / 'tmp_basin_outline.shp')
        shapefile_crs = shapefile.crs
        shape = shapefile[0]['geometry']
        bbox = shapefile.bounds

        # Read the DEM of North America
        NA_DEM = rio.open(basinmaker_root / 'hyd_na_dem_15s.tif')
        ...
        with rio.open(
            project_root / gauge_name / f'{gauge_name}_dem_15s.tif',
            'w',
            driver='GTiff',
        ...
        HRU_output_folder = os.path.join(os.getcwd(), gauge_name, 'HRU_simplified')
        ...
        bm.Generate_HRUs(
            ...
            path_subbasin_polygon         = os.path.join(drainage_area_folder, "finalcat_info_" + version_number + ".shp"), 
            ...
            path_to_dem= str(project_root) + f'/{gauge_name}' + f'/{gauge_name}_dem_15s.tif',
            ...
            pixel_size = 90         # User-defined grid size in m. We recommend using 30 m for OLRRP and 90 m for NA. The unit follows the coordinate system of the routing network polygons.
        )
        ...
        # define another folder that will save the outputs 
        raven_model_dir = os.path.join(os.getcwd(), gauge_name, 'Raven_inputs')

        bm = basinmaker.postprocess()

        bm.Generate_Raven_Model_Inputs(
            path_hru_polygon         = os.path.join(HRU_output_folder, "finalcat_hru_info.shp"),
            model_name            = gauge_name,                             # This is used for naming the output files.
        ...
        raven_model_dir = os.path.join(os.getcwd(), gauge_name, 'Raven_inputs', 'RavenInput')


        # Delete the automatically-generated empty rvp file
        shutil.rmtree(raven_model_dir + '/obs')
        os.remove(os.path.join(raven_model_dir , f'{gauge_name}.rvp'))


        '''Create the SR_LSTM required input files'''
        print('Create the SR_LSTM required input files...')
        srlstm_output_folder = Path(os.getcwd() + f'/{gauge_name}_{exp_name}')
        os.makedirs(srlstm_output_folder)

        basin_files = os.listdir(drainage_area_folder)
        raven_files = os.listdir(raven_model_dir)

        for file in basin_files:
            shutil.copy(Path(drainage_area_folder + '/' + file), Path(srlstm_output_folder / file))

        for file in raven_files:
            shutil.copy(Path(raven_model_dir + '/' + file), Path(srlstm_output_folder / file))

        # check the rvh file make sure the gauged subbasin is marked as 1 not 0
        with open(srlstm_output_folder / f'{basin_id}.rvh', 'r') as f:
        ...
        for li in range(start_index, end_index):
            ...
            if downstream_id == '-1':
                ...
                if gauged_indicator == '0':
                    ...
                    with open(srlstm_output_folder / f'{basin_id}.rvh', 'w') as f:
                        f.writelines(alllines)


        #shutil.rmtree(basinmaker_root / gauge_name)
        shutil.rmtree(project_root / gauge_name)
        ...
        if not os.path.isfile(project_root / f'data/discharge_obs/{basin_id}_discharge.csv'):
            ...
            shutil.rmtree(srlstm_output_folder)
            sys.exit(1)
        ...
        shutil.move(str(srlstm_output_folder), project_root / 'data', copy_function = shutil.copytree)
